...
def draw_scanline(img, y_pos: int, color=None, thickness=2):
    ...
    img[:] = cv2.addWeighted(img, 0.95, overlay, 0.25, 0)
    return img
...
def draw_status_panel(img, lines, pos=(10, 60), bg_color=(10,10,20), alpha=0.6):
    ...
    img[:] = cv2.addWeighted(img, 1.0, overlay, alpha* 0.4, 0)
...
def draw_top3_emotions(img, top3, bbox, font_scale=0.55, line_height=18):
    """
    Draws the top 3 emotions with their percentages near the face bounding box.

    top3: List[ (label, percent_float) ]
    bbox: (x, y, w, h)
    """
    x, y, w, h = bbox

    # Panel position (to the right of the face box)
    px = x + w + 15
    py = y + 5

    # background panel
    panel_w = 150
    panel_h = 20 + line_height * len(top3)

    overlay = img.copy()
    cv2.rectangle(overlay, (px - 6, py - 20), (px - 6 + panel_w, py - 20 + panel_h),
                  (10, 10, 25), -1)  # dark translucent panel
    img[:] = cv2.addWeighted(img, 1.0, overlay, 0.45, 0)

    # Title
    cv2.putText(img, "EMOTIONS",
                (px, py - 5),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, NEON_ACCENT, 1, cv2.LINE_AA)

    # List each emotion
    for i, (lab, pct) in enumerate(top3):
        text = f"{lab.capitalize():<10} {pct:.1f}%"
        ty = py + i * line_height
        cv2.putText(img, text,
                    (px, ty),
                    cv2.FONT_HERSHEY_SIMPLEX, font_scale, NEON_CYAN, 1, cv2.LINE_AA)
